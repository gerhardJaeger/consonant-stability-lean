...
fam <- "data/families/Bantu/"
d <- read_tsv("data/series_counts.tsv")

tree <- read.nexus(str_c(fam, "phylogeny/ctmc4g-asc.ucln.yule.ba-sp.tree")) %>%
    multi2di()

conversion <- read_csv(str_c(fam, "phylogeny/Bantu_Glottocodes_Koile.csv"))
...
ggtree(tree) +
  geom_tiplab() +
  geom_point(data = merged_df, aes(color = series), size = 5) +
  scale_color_continuous(low = "green", high = "red") +
  geom_text(data = merged_df, aes(label = round(series, 2))) +
  theme_tree2()
