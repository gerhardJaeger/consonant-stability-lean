...
transformed data {
   ...
   for (i in 1:Ndata) {
    // y_transformed[i] = log(y[i] + 0.001);
     y_transformed[i] = __TRANSFORMATION__;
   }
}
...
generated quantities {
  ...
  for (i in 1:Ndata) {
    log_lik[i] = __Y_LIKELIHOOD__;
    // y_rep[i] = exp(normal_rng(z[i], tau))-0.001;
    y_rep[i] = __Y_REP_GENERATOR__;
  }
  ...
  // Prior predictive samples: draw z_prior from prior
  {
    ...
    for (i in 1:Ndata) {
      ...
      y_prior[i] = __Y_PRIOR_GENERATOR__;
    }
  }
}
