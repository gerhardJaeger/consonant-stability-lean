...
generated quantities {
  ...
  vector[Ndata] log_lik_y;   // <-- NEW: on original y-scale (with Jacobian)
  ...
  for (i in 1:Ndata) {
    ...
    log_lik_y[i] = lognormal_lpdf(y[i] + eps | z[i], tau);    
  ...
  // Prior predictive samples: draw z_prior from prior
  {
    ...
    real mu_prior = normal_rng(1, 0.5);
    ...
    // real tau_prior = exponential_rng(100);
    ...
    for (i in 1:Ndata) {
      y_prior[i] = exp(normal_rng(z_prior[i], tau_prior))-eps;
    }
  }
}
